...
\maketitle
\section{摘要}
RMSNorm 的主要作用是稳定 Transformer 中各层输入向量的整体尺度。可以把它类比成电路里的“稳压器”：前面传来的数值可能忽大忽小，而 RMSNorm 会在尽量保留各个分量相对关系的同时，把整体大小调整到一个比较稳定的范围，从而让模型各层的计算更加稳定。
\section{文本到向量的变化}
...
这四个数的方差是
\[
\sigma^2=\frac{\sum_{i=1}^{n}{(x_i-\mu)^2}}{n}=\frac{(-1.5)^2+(-0.5)^2+0.5^2+1.5^2}{4}=1.25，
\sigma\approx1.118.
\]
...
上面其实就是LayerNorm的核心思想了。而LayerNorm完整定义还会加入两个参数来训练和一个偏移防止除0异常。
...
\begin{itemize}[leftmargin=2em]
    \item $\varepsilon$ 是一个很小的正数，用来避免方差为 0 或非常小时出现除零和数值不稳定；
...
\qquad i=1,\ldots,D.
\]
这里 $\varepsilon$ 是很小的正数，依旧是防止全零输入导致除0异常，$w_i$ 是每个分量的可训练缩放系数，让模型在训练中自己决定每个维度的重要程度。所有通道共用同一个 RMS 分母，但各自有独立的 $w_i$。可以看到，RMS不用做平移，所以只有一个可训练参数，没有LayerNorm的偏移参数。
